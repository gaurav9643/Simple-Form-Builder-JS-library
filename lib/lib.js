(function(){
    var FormBuilder = function(opts){
        ...
        this.defaultsFields = [
            {type:'text',placeholder:'First Name',required:true,fieldName:'firstname',label:'First Name'},
            {type:'text',placeholder:'Last Name',required:true,fieldName:'lastname',label:'Last Name'},
            {type:'text',placeholder:'Email',required:true,fieldName:'email',label:'Email'}
            ];
        if(opts.customFields) {this.formFields = opts.fields} else{ this.formFields = this.defaultsFields};
    ...
    function addEvent(_f){
        _f.mainForm.addEventListener('submit',function(e){
            ...
            //console.log(_fv,_f.options.method);
            ajax.request(
                _f.options.apiUrl,
                _f.options.method,
                _fv,
                function(data){
                    ...
                    }else{console.log('Notification is disabled')}
    ...
    var ajax = {
        ...
        request : function(url,method,data,success,failure,async){
            ...
            //console.log(method === "GET" && typeof data !== 'undefined' || data !== null,url,method,data,success,failure,async)
            ...
            else{failure('Problem in internet connection');}
            //console.log(this.xhr);
        }
    }
    // ajax.request('http://dummy.restapiexample.com/api/v1/employees','GET',null,function(data){
    // console.log(data);
    // },function(){
    //     console.log('error');
    // },true);
